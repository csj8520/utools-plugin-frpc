  ...
  <el-dialog :model-value="modelValue" @close="emit('update:modelValue', false)" title="编辑" style="min-width: 600px">
    <el-form :model="result" ref="form" label-width="100px">
      ...
      <template v-if="result.type === 'http' || result.type === 'https' || result.type === 'tcpmux'">
        <el-form-item label="本地IP" prop="localIP" :required="!enablePlugin">
          <el-input v-model="result.localIP" />
        </el-form-item>
        ...
        <el-form-item
          ...
          required
        >
          <div class="edit__input__group">
            <el-input v-model="result.customDomains[idx]" />
            <el-icon><Close @click="result.customDomains.splice(idx, 1)" /></el-icon>
          </div>
        </el-form-item>
        <el-form-item>
      ...
      <template v-else-if="result.type === 'stcp' || result.type === 'sudp' || result.type === 'xtcp'">
        <el-form-item label="绑定IP" prop="localPort" :required="!enablePlugin">
          <el-input v-model="result.localPort" />
        </el-form-item>
        <el-form-item label="绑定端口" prop="localPort" :required="!enablePlugin">
          <el-input-number v-model="result.localPort" :min="0" :max="65535" placeholder="8080" />
        </el-form-item>
        <!-- <el-form-item v-if="result.type === 'xtcp'" label="NAME" prop="server_name" :required="!enablePlugin">
          <el-input v-model="result.server_name" />
        </el-form-item> -->
        <el-form-item label="密钥" prop="secretKey">
          <el-input v-model="result.secretKey" />
        </el-form-item>
        <!-- <el-form-item label="角色" prop="allowUsers">
          <el-input v-model="result.allowUsers" />
        </el-form-item> -->
      </template>
...
const props = withDefaults(defineProps<{ modelValue?: boolean; data?: FrpcConfig.Proxie }>(), {
  modelValue: false,
  data: () => ({ type: 'http', name: '', _enable: true, localIP: '127.0.0.1', transport: { useCompression: true, useEncryption: true } })
});
